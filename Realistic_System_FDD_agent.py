...
def grid_search_with_mutex_loss_weighted_sumrate_train_random_0_1(N_rf = 8):
    ...
    session = tf.compat.v1.Session(config=config)
    # fname_template = "trained_models/Sept23rd/Nrf=4/Nrf={}normaliza_input_0p25CE+residual_more_G{}"
    fname_template_template = "trained_models/Feb8th/user_loc0/train_with_0_1_weight/alpha=0p05_NRF={}".format(N_rf)
    fname_template = fname_template_template + "{}"
    # problem Definition
    pre_train = 0
    M = 64
    ...
    np_data = ModelTrainer(save_dir=fname_template.format(".npy"), data_cols=3, epoch=EPOCHS)
    # training loop
    env = Weighted_sumrate_model(K, M, N_rf, N, alpha, hard_decision=False, loss_fn=Sum_rate_utility_WeiCui_stable(K, M, sigma2_n))
    for i in range(0, EPOCHS):
        train_hard_loss.reset_states()
        # generate training data
        train_data = gen_realistic_data("trained_models/Feb8th/user_loc0/one_hundred_user_config_0.npy", N, K, M, Nrf=N_rf)
        ...
        for e in range(0, rounds):
            with tf.GradientTape(persistent=True) as tape:
                ...
                loss = weight_sr + mutex_loss_fn(raw_ans[:, -1]) + 0.5 * tf.square(weight - out) * (1.0 - weight)
...
if __name__ == "__main__":
    ...
    for N_rf_to_search in [7,6,5,4,3,2,1]:
