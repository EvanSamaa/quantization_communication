...
def train_step(features, labels, N=None, epoch=0):
    with tf.GradientTape(persistent=True) as tape:
        ...
        for i in range(0, scheduled_output.shape[1]):
            ...
            loss_4 = loss_4 + 0.1 * tf.exp(tf.constant(-scheduled_output.shape[1]+1+i, dtype=tf.float32)) * (mse + ce)
...
if __name__ == "__main__":
    ...
    fname_template = "trained_models/SEPT30th/Nrf=4/Nrf={}trained_G_data_CE+MSE{}"
    check = 500
    SUPERVISE_TIME = 0
    training_mode = 2
    swap_delay = check / 2
