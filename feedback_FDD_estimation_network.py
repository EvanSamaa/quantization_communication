...
def train_step(features, labels, N=None, epoch=0, lr_boost=1.0):
    with tf.GradientTape(persistent=True) as tape:
        ...
        loss_3 = 10*tf.keras.losses.MeanSquaredError()(reconstructed_input, tf.abs(input_mod))
    ...
    gradients_2 = tape.gradient(0.1 * loss_4, model.get_layer("model").trainable_variables)
    print(gradients_2)
    optimizer.apply_gradients(zip(gradients_2, model.get_layer("model").trainable_variables))
...
if __name__ == "__main__":
    ...
    fname_template = "trained_models/OCT30/Nrf=4/seeding={}one_hot+feedback{}"
    check = 250
    SUPERVISE_TIME = 0
    training_mode = 2
    swap_delay = check / 2
