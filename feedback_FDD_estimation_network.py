...
def train_step(features, labels, N=None, epoch=0):
    with tf.GradientTape(persistent=True) as tape:
        ...
        for i in range(0, scheduled_output.shape[1]):
            ...
            loss_4 = loss_4 + 10*tf.exp(tf.constant(-scheduled_output.shape[1]+1+i, dtype=tf.float32)) * ce
...
if __name__ == "__main__":
    ...
    fname_template = "trained_models/Sept14th/perfect_CSI_fixed_concat_X_compress_XG/Perfect_CSI Nrf={}, 2x128_per_linkx6_alt+10x_CE_loss{}"
    check = 500
    SUPERVISE_TIME = 0
    training_mode = 2
    swap_delay = check / 2
