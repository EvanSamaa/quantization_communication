...
def train_step(features, labels, N=None, epoch=0, lr_boost=1.0, reg_strength = 1.0):
    with tf.GradientTape(persistent=True) as tape:
        ...
        # loss = loss_2 + loss_3
        loss = reg_strength * loss_4 + 0.01 * loss_1
...
if __name__ == "__main__":
    ...
    fname_template = "trained_models/Nov_15/mse_loss_argmax+no_iternum{}"
    check = 250
    SUPERVISE_TIME = 0
    training_mode = 2
    swap_delay = check / 2
    ...
    for j in Es:
        for i in mores:
            ...
            model = FDD_per_link_archetecture_more_G(M, K, 30, N_rf, True, max_val)
