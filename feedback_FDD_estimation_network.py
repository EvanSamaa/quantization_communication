...
def train_step(features, labels, N=None, epoch=0):
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        loss_4 = tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True)(tf.argmax(per_user_softmaxes, axis=2), per_user_softmaxes)
        loss_4 = loss_4 + tf.keras.losses.CategoricalCrossentropy()(overall_softmax, mask)
...
if __name__ == "__main__":
    fname_template = "trained_models/Aug27th/512x1_Per_User_Schedular+fine_grain_CE+MP{}"
    check = 500
    SUPERVISE_TIME = 0
    training_mode = 2
    swap_delay = check / 2
    ...
    model = FDD_per_user_architecture_double_softmax_all_softmaxes(M, K, 4, N_rf, True)
