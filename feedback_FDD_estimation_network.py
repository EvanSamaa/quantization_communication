...
def train_step(features, labels, N=None, epoch=0, lr_boost=1.0, reg_strength = 1.0):
    with tf.GradientTape(persistent=True) as tape:
        ...
        scheduled_output, raw_output, input_mod, input_reconstructed_mod, reconstructed_input = model(features)
        # loss_1 = tf.keras.losses.MeanSquaredError()(reconstructed_input, tf.abs(features))
        loss_1 = tf.reduce_mean(sum_rate_train(scheduled_output[:, -1], reconstructed_input * max_val))
        ...
        loss_3 = loss_3 + tf.keras.losses.MeanSquaredError()(input_reconstructed_mod, input_mod)
        ...
        for i in mores:
            ...
            for epoch in range(EPOCHS):
                ...
                if epoch % check == 0:
                    ...
                    scheduled_output, raw_output, input_mod, input_reconstructed_mod, reconstructed_input = model.predict(valid_data, batch_size=N)
