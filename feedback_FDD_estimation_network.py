...
def train_step(features, labels, N=None, epoch=0, lr_boost=1.0, reg_strength = 1.0):
    with tf.GradientTape(persistent=True) as tape:
        ...
        loss_4 = loss_4 + tf.square(tf.reduce_sum(scheduled_output[:, -1], axis=1) - N_rf)
...
if __name__ == "__main__":
    ...
    fname_template = "trained_models/Nov_15/sigmoid+smol_user_reg+Nrf_reg{}"
    check = 250
    SUPERVISE_TIME = 0
    training_mode = 2
    swap_delay = check / 2
