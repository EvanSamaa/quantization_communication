...
def train_step(features, labels, N=None, epoch=0):
    with tf.GradientTape(persistent=True) as tape:
        ...
        mask = tf.stop_gradient(Harden_scheduling_user_constrained(N_rf, K, M)(scheduled_output))
...
if __name__ == "__main__":
    ...
    fname_template = "trained_models\Sept14th\All_input_model/Nrf={}{}_All_input+weighted_CE_loss{}"
    check = 500
    SUPERVISE_TIME = 0
    training_mode = 2
    swap_delay = check / 2
    ...
    for j in Es:
        for i in mores:
            ...
            model = All_info_scheduler(M, K, N_rf)
