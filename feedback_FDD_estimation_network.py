...
def train_step(features, labels, N=None, epoch=0):
    with tf.GradientTape(persistent=True) as tape:
        ...
        for i in range(0, scheduled_output.shape[1]):
            ...
            mask = tf.stop_gradient(Harden_scheduling_user_constrained(N_rf, K, M, default_val=-1)(scheduled_output[:, i]))
...
if __name__ == "__main__":
    ...
    fname_template = "trained_models/Sept14th/perfect_CSI_fixed_concat_X_MSE_Reg/Perfect_CSI Nrf={}, 2x512_per_linkx6_alt+weighted_CE_loss{}"
    check = 500
    SUPERVISE_TIME = 0
    training_mode = 2
    swap_delay = check / 2
