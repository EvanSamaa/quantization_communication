...
def train_step(features, labels, N=None, epoch=0, lr_boost=1.0):
    with tf.GradientTape(persistent=True) as tape:
        ...
        loss_4 = tf.reduce_mean(tf.square(tf.multiply(scheduled_output[:, -1], 1.0-scheduled_output)), axis=1)
...
if __name__ == "__main__":
    ...
    fname_template = "trained_models/Nov_15/new_normalization+less_G_k=12_N_RF{}_3layer+different_reg{}"
    check = 250
    SUPERVISE_TIME = 0
    training_mode = 2
    swap_delay = check / 2
