...
def train_step(features, labels, N=None, epoch=0, lr_boost=1.0, reg_strength = 1.0):
    with tf.GradientTape(persistent=True) as tape:
        ...
        loss_1 = tf.minimum(loss_1, Stochastic_softmax_selectior_and_loss(M, K, N_rf, 100)(raw_output[:, -1], scheduled_output[:, -1], features, sum_rate_train))
...
if __name__ == "__main__":
    ...
    fname_template = "trained_models/Nov_15/stochastic_loss{}"
    check = 250
    SUPERVISE_TIME = 0
    training_mode = 2
    swap_delay = check / 2
