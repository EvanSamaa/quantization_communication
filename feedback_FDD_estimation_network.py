...
def train_step(features, labels, N=None, epoch=0):
    ...
    with tf.GradientTape() as tape:
        ...
        sparse_reconstructed_input = reconstructed_input + tf.stop_gradient(sparse_matrix_from_full(reconstructed_input, 5) - reconstructed_input)
        loss_1 = tf.keras.losses.MeanSquaredError()(sparse_reconstructed_input, sparse_matrix_from_full(tf.abs(features), 5))
