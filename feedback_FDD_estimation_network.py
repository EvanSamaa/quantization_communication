...
def train_step(features, labels, N=None, epoch=0):
    ...
    with tf.GradientTape() as tape:
        ...
        loss_1 = tf.keras.losses.MeanSquaredError()(reconstructed_input, features)
...
if __name__ == "__main__":
    fname_template = "trained_models/Aug25th/B10E10_small_decoder+outerproduct_in+MP{}"
    check = 500
    SUPERVISE_TIME = 0
    training_mode = 2
    swap_delay = check / 2
    ...
    np.random.seed(seed)
    model = CSI_reconstruction_model_seperate_decoders_input_mod(M, K, B, E, N_rf, 6, more=1, qbit=0)
    print(model.summary())
