...
def Autoencoder_Decoding_module(output_size, input_shape, i=0):
    ...
    x = LeakyReLU()(x)
    x = tf.keras.layers.BatchNormalization()(x)
    x = Dense(64, kernel_initializer=tf.keras.initializers.he_normal(), name="decoder_{}_dense_3".format(i))(x)
    x = LeakyReLU()(x)
    x = tf.keras.layers.BatchNormalization()(x)
    x = Dense(64, kernel_initializer=tf.keras.initializers.he_normal(), name="decoder_{}_dense_4".format(i))(x)
    x = LeakyReLU()(x)
    x = tf.keras.layers.BatchNormalization()(x)
...
def CSI_reconstruction_model_seperate_decoders(M, K, B, E, N_rf, k, more=1, qbit=0):
    ...
    inputs_mod = tf.abs(inputs)
    find_nearest_e = Closest_embedding_layer(user_count=K, embedding_count=2 ** B, bit_count=E, i=0)
    encoder = Autoencoder_Encoding_module((K, M), i=0, code_size=E * more + qbit, normalization=True)
