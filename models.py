...
def FDD_reduced_output_space(M, K, N_rf=3):
    ...
    user_selection = tf.divide(user_selection, tf.tile(tf.reduce_max(user_selection, axis=1, keepdims=True), [1, M, 1]))
    ...
    precoder_selection = tf.divide(precoder_selection, tf.tile(tf.reduce_max(precoder_selection, axis=1, keepdims=True), [1, M, 1]))
