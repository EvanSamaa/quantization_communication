...
def train_step(features, labels, N=None, epoch=0):
    ...
    with tf.GradientTape() as tape:
        ...
        loss_4 = OutPut_Limit_onesided(N_rf)(predictions)
        loss = loss_1 + 10000*loss_4
    gradients = tape.gradient(loss, model.trainable_variables)
    optimizer.apply_gradients(zip(gradients, model.trainable_variables))
...
if __name__ == "__main__":
    fname_template = "trained_models/Aug_15th/Feedback_model_hard_baseline+10000x_oneside_N_RF_constraint{}"
    check = 500
    SUPERVISE_TIME = 0
    training_mode = 2
    swap_delay = check/2
