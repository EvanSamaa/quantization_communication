    ...
    for i in range(0, EPOCHS):
        ...
        for e in range(0, rounds):
            ...
            with tf.GradientTape(persistent=True) as tape:
                ...
                train_label = tf.reshape(tf.tile(tf.expand_dims(train_data, axis=0), [sample_size,1, 1, 1]), [sample_size*N, K, M])
                ###################### model post-processing ######################
                loss = train_sum_rate(out, train_label) + 0.01*mutex_loss_fn(raw_ans[:, -1])
...
def grid_search_with_emsemble(N_rf = 8):
    ...
    session = tf.compat.v1.Session(config=config)
    # fname_template = "trained_models/Sept23rd/Nrf=4/Nrf={}normaliza_input_0p25CE+residual_more_G{}"
    fname_template_template = "trained_models/Jan_18/test_DNN_ensemble".format(N_rf)
    fname_template = fname_template_template + "{}"
    check = 250
    SUPERVISE_TIME = 0
    training_mode = 2
    ...
    for i in range(0, EPOCHS):
        ...
        for e in range(0, rounds):
            ...
            with tf.GradientTape(persistent=True) as tape:
                ...
                raw_ans2 = tf.transpose(raw_ans2, [0, 1, 3, 2])
                raw_ans2 = tf.reshape(raw_ans2[:, -1], [N * N_rf, K * M])
