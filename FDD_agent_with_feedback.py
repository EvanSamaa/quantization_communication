...
def grid_search_STD(more = 8):
    ...
    session = tf.compat.v1.Session(config=config)
    # fname_template = "trained_models/Sept23rd/Nrf=4/Nrf={}normaliza_input_0p25CE+residual_more_G{}"
    fname_template_template = "trained_models/Dec28/NRF=5/GNN_annealing_temp_B={}+limit_res=6.h5".format(more)
    fname_template = fname_template_template + "{}"
    check = 250
    SUPERVISE_TIME = 0
    training_mode = 2
    ...
    check = 200
    # model = FDD_per_link_archetecture_more_G(M, K, 7, N_rf, True, max_val)
    model = FDD_per_link_archetecture_more_G(M, K, B, E, N_rf, 4, more=more, avg_max=max_val)
    optimizer = tf.keras.optimizers.Adam(lr=lr)
    ################################ Metrics  ###############################
    sum_rate = Sum_rate_utility_WeiCui(K, M, sigma2_n)
    train_sum_rate = Sum_rate_utility_WeiCui(K, M, sigma2_n)
...
if __name__ == "__main__":
    for N_rf_to_search in [65,129,2]:
